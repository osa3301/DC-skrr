...
void c_misc::rapid_fire( )
{
	/*
	if ( !g_cl.m_local && g_cl.m_local->alive())
		return;

	if ( g_vars.misc.tick_base_manipulation_rapidfire_key != 0 )
	{
		static bool is_clicked = false;
		static bool is_down = false;

		bool key_pressed = g_input.key_pressed( g_vars.misc.tick_base_manipulation_rapidfire_key );
		is_clicked = !key_pressed && is_down;
		is_down = key_pressed;

		if (key_pressed)
			g_cl.m_tickbaseshift = TIME_TO_TICKS(2.0f);
		else
			g_cl.m_tickbaseshift = TIME_TO_TICKS(0.0f);
	}
	*/
}

void c_misc::viewmodel_aimbot( c_view_setup* vsView )
{
	if ( !g_cl.m_local )
		return;

	static bool first = true;
	static vec3_t oAngle = vec3_t(0, 0, 0);
	static float timeremaining = 1000.f;
	static float maxtime = 0.4f;

	if (!(*g_cl.aim_angles).is_valid() || !(*g_cl.original_aim_angles).is_valid())
		return;

	if ( (vsView->m_angles - *g_cl.aim_angles ).length( ) < 1.f || timeremaining < 0.f )
	{ 
		*g_cl.aim_angles = *g_cl.original_aim_angles = oAngle = vec3_t( 0, 0, 0 );
		first = true;
	...
	auto vm = reinterpret_cast<c_csplayer*>( g_csgo.m_entity_list->get_client_entity_from_handle( local->viewmodel_handle( ) ) );

	if ( !vm ) 
		return;

	if (first || oAngle != *g_cl.original_aim_angles)
	{ // if we just set the angle or if we started aimboting someone else reset
		oAngle = *g_cl.aim_angles;
		first = false;
		timeremaining = maxtime;
	}

	auto deltaAngle = *g_cl.aim_angles - vsView->m_angles;

	math::clamp_angles(deltaAngle);

	vm->set_abs_angles(vsView->m_angles + deltaAngle);

	timeremaining -= g_csgo.m_global_vars->m_absolute_frametime;

	//g_cl.aim_angles = math::lerp(vsView->m_angles, oAngle, timeremaining / maxtime);
	math::clamp_angles(*g_cl.aim_angles);
}
...
void c_misc::transparent_props( float override_transparency ) {
	if( g_csgo.m_engine && !g_csgo.m_engine->is_connected( ) || !g_cl.m_local )
		return;
...
void c_misc::flashlight( ) {
	...
	if ( g_input.key_pressed( g_vars.misc.flashlight_key ) && GetTickCount64( ) > last_tick + 250 ) {
		if ( !flashlight ) {
