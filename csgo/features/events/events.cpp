...
void c_event_listener::fire_game_event( i_game_event *m_event ) {
	...
	if( !strcmp( m_event->get_name( ), "bullet_impact" ) ) {
		...
		if( g_vars.misc.bullet_impacts )
			g_csgo.m_debug_overlay->add_box_overlay( position, vec3_t( -2, -2, -2 ), vec3_t( 2, 2, 2 ), vec3_t( 0, 0, 0 ), 255, 140, 0, 100, g_vars.misc.bullet_impacts_duration );
		...
		if( g_vars.visuals.impact ) {
			...
			beam_info.m_red = 0.f;
			beam_info.m_green = 255.f;
			beam_info.m_blue = 0.f;
	...
	if( !strcmp( m_event->get_name( ), "player_hurt" ) ) {
		auto attacker = g_csgo.m_engine->get_player_for_user_id( m_event->get_int( "attacker" ) );
		auto target = g_csgo.m_engine->get_player_for_user_id( m_event->get_int( "userid" ) );
		...
		if( attacker == g_csgo.m_engine->get_local_player( ) && target != g_csgo.m_engine->get_local_player( ) ) {
			...
			if( g_csgo.m_engine->get_player_info( target, &info ) && ent ){
				if( g_vars.misc.hitmarker_sound ) {
					...
					vec3_t enemy_pos = ent->abs_origin();
				...
				{
					//g_notify.add(true, OSHGui::Drawing::Color::FromARGB(220, 255, 140, 0), "hit %s in the %s for %i damage.", s.c_str(),
		...
		if( g_csgo.m_engine->get_player_info( user, &info ) ){
			...
			{
				//g_notify.add(true, OSHGui::Drawing::Color::FromARGB(220, 140, 255, 0), "%s bought %s", info.m_player_name, m_event->get_string("weapon"));
			}
		}
	}
