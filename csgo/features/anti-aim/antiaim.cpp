...
void c_antiaim::adjust_yaw( c_user_cmd *ucmd ) {
	...
	/*
	if( g_input.key_pressed( g_vars.antiaim.side_switch_key ) && last_tickcount < int( GetTickCount64( ) ) ) {
		side = !side;
		last_tickcount = int( GetTickCount64( ) ) + 250;
	}
	*/

	if( !g_cl.m_sendpacket && !g_csgo.m_clientstate->m_choked_commands ) {
		switch( (int)g_vars.antiaim.yaw ) {
			case 1:
				if( side )
...
void c_antiaim::adjust_pitch( c_user_cmd *ucmd ) {
	switch( (int)g_vars.antiaim.pitch ) {
		case 1:
			m_input.x = 89.f;
			break;
		default:
