...
{
	/*
	typedef bool( __fastcall* TraceToExitFn )(vec3_t&, const vec3_t&, const vec3_t&, trace_t*, trace_t*);
	static TraceToExitFn trace_to_exit = ( TraceToExitFn )pattern::find( g_csgo.m_engine_dll, "55 8B EC 83 EC 30 F3 0F 10 75" );

	if (!trace_to_exit)
		return false;

	return trace_to_exit(end, start, dir, enter_trace, exit_trace);
	*/
	float dist = 0.f;
