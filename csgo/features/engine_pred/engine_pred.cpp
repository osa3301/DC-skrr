...
int c_engine_pred::post_think( c_base_player *player ) const {
	...
	if( player->alive( ) ) {
		...
		util::misc::vfunc< void( __thiscall *)( void * ) >( player, 339 )( player ); //UpdateColissionBounds 338 
		...
		if( *reinterpret_cast< int * >( uintptr_t( player ) + 0x28BC ) == -1 )
			util::misc::vfunc< void( __thiscall *)( void *, int ) >( player, 218 )( player, 0 ); //SetSequence 218 

		util::misc::vfunc< void( __thiscall *)( void * ) >( player, 219 )( player ); //StudioFrameAdvance 219 

		PostThinkVPhysics( player );
	}
	SimulatePlayerSimulatedEntities( player ); // Exception here, update sigs fgt pls
...
void c_engine_pred::start( c_user_cmd *ucmd ) {
	...
	if( !m_prediction_player || !m_prediction_seed ) {
		...
		//m_prediction_seed = **reinterpret_cast<int**>(pattern::find("client_panorama.dll", sig_prediction_random_seed) + 2);
