...
namespace hook {
	namespace fn {
		using setCursorPos_t = BOOL (__stdcall *)( int, int );
		...
		using WriteUsercmdDeltaToBuffer_hook_t = bool( __fastcall * )( void*, int, bf_write*, int, int, bool );
	};
	...
	HRESULT __stdcall Present( IDirect3DDevice9Ex *device, const RECT *pSourceRect, const RECT *pDestRect, HWND hDestWindowOverride, const RGNDATA *pDirtyRegion );
	...
	bool __fastcall WriteUserCmdDeltaToBuffer_hook( void* ECX, int nSlot, bf_write* buf, int from, int to, bool isNewCmd );
};
...
extern hook::fn::setCursorPos_t oSetCursorPos;
extern hook::fn::Present_t oPresent;
extern hook::fn::Reset_t oReset;