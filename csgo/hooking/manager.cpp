...
bool c_hooks::hook( ) {
	...
	oWndProc = (WNDPROC)SetWindowLongA(g_vars.global.hWindow, GWL_WNDPROC, (LONG_PTR)&hook::Hooked_WndProc);

	//oReset = *reinterpret_cast<decltype(hook::Reset)*>(g_csgo.reset);
	//*reinterpret_cast<decltype(hook::Reset)**>(g_csgo.reset) = hook::Reset;

	//oPresent = *reinterpret_cast<decltype(hook::Present)*>(g_csgo.present);
	//*reinterpret_cast<decltype(hook::Present)**>(g_csgo.present) = hook::Present;

	//oSetCursorPos = *reinterpret_cast<decltype(hook::setCursorPos)*>(g_csgo.setcursorpos);
	//*reinterpret_cast<decltype(hook::setCursorPos)**>(g_csgo.setcursorpos) = hook::setCursorPos;

	//oReset = (hook::fn::Reset_t)((new c_vmt((PDWORD*)g_csgo.reset))->hook_method((DWORD)&hook::Reset, 0));

	//oPresent = (hook::fn::Present_t)((new c_vmt((PDWORD*)g_csgo.present))->hook_method((DWORD)&hook::Present, 0));

	//oSetCursorPos = (hook::fn::setCursorPos_t)((new c_vmt((PDWORD*)g_csgo.setcursorpos))->hook_method((DWORD)&hook::setCursorPos, 0));
	...
	/*
	if (!m_client.hook_method(hook::idx::WRITE_USERCMD_TO_DELTA, &hook::WriteUserCmdDeltaToBuffer_hook)) {
		_RPTF0(_CRT_ERROR, "Failed to hook WriteUserCmdDeltaToBuffer_hook. This is fatal.");
		return false;
	}
	*/
